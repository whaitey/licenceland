...
class LicenceLand_CD_Keys {
    ...
    public function assign_cd_key_to_order_item($item, $cart_item_key, $values, $order) {
        // Assign keys ONLY on Primary. Secondary mirrors to Primary which assigns and emails.
        if (function_exists('licenceland') && licenceland()->sync && method_exists(licenceland()->sync, 'is_primary_site')) {
    ...
    public function handle_backorder_creation($item, $cart_item_key, $values, $order) {
        // Backorders are tracked on Primary only
        if (function_exists('licenceland') && licenceland()->sync && method_exists(licenceland()->sync, 'is_primary_site')) {
            if (!licenceland()->sync->is_primary_site()) {
                return;
            }
...
%s', 'licenceland'),
    ...
    public function process_backorders_on_stock_update($post_id) {
        // Only Primary processes backorders
        if (function_exists('licenceland') && licenceland()->sync && method_exists(licenceland()->sync, 'is_primary_site')) {
            if (!licenceland()->sync->is_primary_site()) {
                return;
            }
